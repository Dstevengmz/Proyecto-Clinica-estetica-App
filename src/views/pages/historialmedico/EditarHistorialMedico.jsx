...
import Alerta from "../../../assets/js/alertas/historialmedico/HistorialMedico"
...
function EditarHistorialMedico() {
  ...
  // Definimos los estados dinámicos
  ...
  useEffect(() => {
    ...
    if (!token) {
       alerta.alertaErrorEditarHistorialMedico("No estás autenticado");
      navigate("/");
      return;
    }
    ...
    const obtenerHistorial = async () => {
      try {
        ...
        const ownerId = response.data?.usuario?.id;
        if (rol !== "doctor" && ownerId && usuarioPerfil?.id && ownerId !== usuarioPerfil.id) {
          alerta.alertaNoSePudoCrearHistorialMedico("No autorizado para editar este historial.");
          navigate(`/mihistorialclinico/${usuarioPerfil?.id ?? ""}`);
          return;
        }
        setFormulario(response.data);
        setEnfermedadesLista(
          response.data.enfermedades
            ? response.data.enfermedades.split(", ").filter(Boolean)
            : []
        );
        setAlergiasLista(
          response.data.alergias
            ? response.data.alergias.split(", ").filter(Boolean)
            : []
        );
        setMedicamentosLista(
          response.data.medicamentos
            ? response.data.medicamentos.split(", ").filter(Boolean)
            : []
        );
        setCirugiasLista(
          response.data.cirugias_previas
            ? response.data.cirugias_previas.split(", ").filter(Boolean)
            : []
        );
        setCondicionesLista(
          response.data.condiciones_piel
            ? response.data.condiciones_piel.split(", ").filter(Boolean)
            : []
        );

        setEnfermedadesHabilitado(
          response.data.enfermedades && response.data.enfermedades.length > 0
        );
        setAlergiasHabilitado(
          response.data.alergias && response.data.alergias.length > 0
        );
        setMedicamentosHabilitado(
          response.data.medicamentos && response.data.medicamentos.length > 0
        );
        setCirugiasHabilitado(
          response.data.cirugias_previas &&
            response.data.cirugias_previas.length > 0
        );
      } catch (error) {
        console.error("Error al cargar el historial médico:", error);
        alerta.alertaNoSePudoCrearHistorialMedico("No se pudo cargar la información del historial médico");
      }
    };

    obtenerHistorial();
  }, [id, cargandoUsuario, rol, usuarioPerfil?.id, navigate]);
  ...
  const actualizarHistorialMedico = async (e) => {
    ...
    try {
      ...
      alerta.alertaHistorialMedicoCreadoExito();
      const isDoctor = (rol || "").toLowerCase() === "doctor";
      if (isDoctor) {
        navigate("/consultarhistorialmedico");
      } else {
        const targetUserId = usuarioPerfil?.id || formulario?.usuario?.id;
        if (targetUserId) {
          navigate(`/mihistorialclinico/${targetUserId}`);
        } else {
          navigate("/consultarhistorialmedico");
        }
      }
      ...
      alerta.alertaNoSePudoEditarHistorialMedico("No se pudo actualizar el historial médico");
    } finally {
      setEnviando(false);
    }
  };
